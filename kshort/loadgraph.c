...
/*============================================================================*/
void LoadGraph(Graph *graph, char *fileName, int inode, int fnode) {
  ...
  /* the best path and the node name                                  */
...
/*graph->node = free (sizeof (graph->node[0])*graph->numberNodes);
graph->arc = free (sizeof (graph->arc[0])*graph->numberArcs);
graph->arcIn = free(sizeof ((graph->arcIn)[0])*graph->numberArcs);
graph->arcOut = free(sizeof ((graph->arcOut)[0])*graph->numberArcs);*/
}
